...
class JobShopParams:
    ...
    def __init__(self, machines: Iterable, jobs: Iterable, p_times: dict, seq: dict, setup: dict, tandas: Iterable, p_times_tandas:dict):
        ...
        self.setup = setup
        self.tandas = tandas
        self.p_times_tandas = p_times_tandas

class JobShopRandomParams(JobShopParams):
    
    def __init__(self, n_machines: int, n_jobs: int, n_tandas: int, t_span=(1, 20), seed=None, t_span_setup=(1,5)):
        ...
        tandas=np.arange(n_tandas,dtype=int)
        p_times_tandas = self.p_times_tandas(machines, jobs, p_times, tandas)
        seq = self._random_sequences(machines, jobs)
        setup = self._random_setup(machines, jobs, t_span_setup)
        super().__init__(machines, jobs, p_times, seq, setup, tandas, p_times_tandas)
    ...
    def p_times_tandas(self, machines, jobs, p_times, tandas):
        return {
            (m, j, t): p_times.get((m,j))/len(tandas)
            for m in machines
            for j in jobs
            for t in tandas
        }
    ...
    def printParams(self):
        ...
        print("los tiempos de proceso para cada tanda son: ")
        print(self.p_times_tandas)
