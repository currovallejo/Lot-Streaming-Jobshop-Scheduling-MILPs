...
def gurobiModel(params:JobShopRandomParams, demand:dict, shift_time:int,maxSolverTime:int): #builds and solve the model
    ...
    while b==0:
        ...
        model.addConstrs(gp.quicksum((s[m,j]+p[m,j,u])*W[m,o,j,u] for j in jobs for u in lotes)<=shift_time for m, o in doble_mo) #14
...
def main():
    ...
    demand={0:300,1:100,2:50,3:200,4:300,5:200} 
    params = getJobShopParameters(machines=2, jobs=3, batches=3, seed=7)
    shift_time=400
