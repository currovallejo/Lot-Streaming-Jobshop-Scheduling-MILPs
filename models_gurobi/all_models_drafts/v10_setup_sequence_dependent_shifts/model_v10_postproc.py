...
def build_model_v10(params:JobShopRandomParams, demand:dict, shift_time:int, power:dict):
    ...
    while b==0 and n_lotes<n_slots:
        ...
        model.setObjective(0.5*E+0.5*C/100,gp.GRB.MINIMIZE)
    ...
    # set upper bound for makespan and energy consumption
    C.ub = C.X
    E.ub = E.X

    # minimize number of lots
    n_lots = model.addVar(vtype=gp.GRB.CONTINUOUS, name = 'nQ')
    ...
    # minimize earliness
    sum_x = model.addVar(vtype=gp.GRB.CONTINUOUS, name = 'sX')    
    model.addConstr(sum_x==gp.quicksum(x[m,j,u] for m,j,u in triple_mju if j!=0 and j!=jobs[-1])) #15
    model.setObjective(sum_x,gp.GRB.MINIMIZE)
    model.optimize()
