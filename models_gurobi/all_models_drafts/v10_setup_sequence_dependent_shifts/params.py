...
class JobShopRandomParams(JobShopParams):
    ...
    def printProcessTimes(self):
        ...
        jobs = [f'Job {i+1}' for i in range(n_columns)]
        machines = [f'Machine {j}' for j in range(n_rows)]

        df = pd.DataFrame(matrix, columns=jobs, index=machines)
    ...
    def printSetupTimes(self):
        ...
        writer = pd.ExcelWriter('setup_times.xlsx', engine='xlsxwriter')
        for m in self.machines:
            ...
            df.to_excel(writer, sheet_name=f'Machine_{m}')
